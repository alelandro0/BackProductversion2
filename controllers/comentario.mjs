...
export const verComentarios = async (req, res) => {
  ...
  const { _id, publicationId } = req.params; // Obtener los parámetros de la URL
  console.log("id del usuario",_id, " id publicacion", publicationId);
  console.log("inicio ver comentario");
  try {
    ...
    res.status(200).json({ comentarios: publication.comentario });
  } catch (error) {
